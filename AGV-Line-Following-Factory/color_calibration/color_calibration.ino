...
void setup() {
  ...
  alvik.color_calibration(0);
  
  Serial.println("Place your robot on blue and send a char");
  while(Serial.available()==0);
    while(Serial.available()>0){
    Serial.read();
  }
  alvik.color_calibration(8);

  Serial.println("Place your robot on red and send a char");
  while(Serial.available()==0);
    while(Serial.available()>0){
    Serial.read();
  }
