...
class SimGUI:
    ...
    def _draw_frame(self, t: float) -> None:
        ...
        for v in range(inst.num_agvs):
            ...
            col  = AGV_COLORS[v % len(AGV_COLORS)]
            load = self._load_at(v, t)
            self.canvas.create_oval(px - AGV_R, py - AGV_R,
                                     px + AGV_R, py + AGV_R,
                                     fill=col, outline="white", width=2, tags="anim")
            self.canvas.create_text(px, py - 5, text=str(v),
                                     font=("", 7, "bold"), fill="white", tags="anim")
            self.canvas.create_text(px, py + 6, text=f"{load}/{cap}",
                                     font=("", 6), fill="white", tags="anim")
    ...
    def _load_at(self, v: int, t: float) -> int:
        """Return the held quantity for AGV v at simulation time t."""
        evs  = self.agv_load_evs[v] if v < len(self.agv_load_evs) else []
        load = 0
        for ev_t, ld in evs:
            if ev_t <= t:
                load = ld
    ...
    def _build_trajectories(self) -> None:
        ...
        self.node_evs      = {}
        self.agv_load_evs  = [[] for _ in range(n_agvs)]
        ...
        for ev in self.events:
            ...
            elif kind == "load_change":
                agv, load = ev[2], ev[3]
                if agv < n_agvs:
                    self.agv_load_evs[agv].append((t, load))
    ...
    def _update_agv_label(self) -> None:
        ...
        for v in range(self.instance.num_agvs):
            ...
            load = self._load_at(v, t)
            lines.append(f"AGV {v}: {action}  [{load}/{cap}]")
