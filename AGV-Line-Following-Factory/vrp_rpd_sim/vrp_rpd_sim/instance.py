...
class Instance:
    ...
    Internal indexing is (row, col) where row 0 is at the TOP. For physical
    interpretation matching a drawing with origin at bottom-left:
        x = col
        y = (grid_rows - 1) - row
    The `node_to_xy` and `xy_to_node` methods do this conversion.

    When node_xy_positions is provided (64 [x, y] pairs in code-array order,
    row 0 first), distance() and edge_time() use real measured coordinates
    ...
    def depot_node(self) -> int:
        return self.depot_row * self.grid_cols + self.depot_col

    def node_to_rc(self, node: int) -> tuple[int, int]:
        return node // self.grid_cols, node % self.grid_cols
    ...
    def set_node_positions(self, positions: list[list[float]]) -> None:
        ...
        positions: flat list of 64 [x, y] pairs in code array order
        (node 0 = top-left, node 63 = bottom-right of the 8x8 grid).
        """
    ...
    def random_instance(
        ...
        depot = depot_row * grid_cols + depot_col
...
def load_positions_xlsx(
    ...
    # Find the data block: scan for cells parseable as coordinates
    # and find the bounding box of numeric data.
    ...
    min_r = min(r for r, _ in data_cells)
    max_r = max(r for r, _ in data_cells)
    min_c = min(c for _, c in data_cells)
    max_c = max(c for _, c in data_cells)
...
def _grid_to_flat(
    ...
    depot_row_first=True: user grid row 0 is the depot (bottom of physical
    grid), which corresponds to code array row (grid_rows-1). Flip rows so
    that code array row 0 = top of physical grid.
    """
    ordered = list(reversed(grid)) if depot_row_first else grid
